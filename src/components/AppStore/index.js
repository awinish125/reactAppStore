...
class AppStore extends Component {
  state = {activeCat: tabsList[0].tabId, search: ''}

  getFilteredApp = () => {
    const {activeCat, search} = this.state
    const filteredApps = appsList.filter(each => each.category === activeCat)
  ...
  render() {
    const {search, activeCat} = this.state
        ...
        <ul className="tabList-container">
          {tabsList.map(each => (
            <TabItem
              key={each.tabId}
              tabList={each}
              updateAppId={this.updateAppId}
              activated={each.tabId === activeCat}
            />
          ))}
        </ul>
